...
// --- CLI Session accessors (multi-session support) ---
...
/**
 * List all CLI sessions, ordered by last used time.
 */
...
/**
 * Get the current (most recently used) CLI session ID.
 */
...
/**
 * Get the current CLI session info.
 */
...
/**
 * Find a CLI session by SDK session ID.
 */
export function findCLISessionBySDK(sdkSessionId: string): CLISessionInfo | undefined {
  const row = db
    .prepare(
      `SELECT id, sdk_session_id, name, summary, first_prompt,
              created_at, last_used_at, is_active, message_count
...
/**
 * Select/switch to a CLI session by short ID or full SDK ID.
 */
export function selectCLISession(sessionId: string): CLISessionInfo | undefined {
  const row = db
    .prepare(
      `SELECT id, sdk_session_id, name, summary, first_prompt,
              created_at, last_used_at, is_active, message_count
  ...
  const now = new Date().toISOString();
  db.prepare(`UPDATE cli_sessions SET last_used_at = ? WHERE id = ?`).run(now, row.id);

  // Also update the sessions table so the orchestrator uses this session
...
export function createCLISession(
  ...
  // Handle potential short-ID collisions by extending the prefix
...
/**
 * Update a CLI session with new metadata.
 */
  ...
  db.prepare(
    `UPDATE cli_sessions
     SET summary = COALESCE(?, summary),
         ...
         last_used_at = ?
     WHERE sdk_session_id = ?`,
  ).run(updates.summary ?? null, updates.messageCount ?? null, now, sdkSessionId);
}

/**
 * End (archive) the current CLI session.
 */
export function endCurrentCLISession(): CLISessionInfo | undefined {
  const current = getCurrentCLISession();
  if (!current) return undefined;

  db.prepare(`UPDATE cli_sessions SET is_active = 0 WHERE sdk_session_id = ?`).run(
    current.sdkSessionId,
  );

  // Clear the sessions table entry for cli
