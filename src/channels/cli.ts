...
export interface CLIRequest {
  id: string;
  type: 'message' | 'list_sessions' | 'select_session' | 'end_session' | 'status';
  timestamp: string;
  text?: string;
  options?: {
    sessionId?: string;
...
export class CLIChannel implements Channel {
  ...
  // Session sync callback - called when newSessionId is received
  private onSessionSync?: (sessionId: string, messageCount: number) => void;
  ...
  /**
   * Set callback for session sync notifications from orchestrator.
   */
  setOnSessionSync(callback: (sessionId: string, messageCount: number) => void): void {
    this.onSessionSync = callback;
  }
  ...
  async connect(): Promise<void> {
    ...
    this.onChatMetadata('cli:default', new Date().toISOString(), 'CLI', 'cli', false);
    ...
    try {
      ...
      switch (request.type) {
        ...
        case 'status':
          ...
          break;
        default:
          this.writeError(request.id, `Unknown request type: ${(request as any).type}`);
      }

      logger.debug({ requestId: request.id, type: request.type }, 'CLI request processed');
    } catch (err) {
  ...
  /**
   * Handle a message request - trigger agent processing.
   */
  private handleMessage(request: CLIRequest): void {
    ...
    this.onMessage('cli:default', msg);
  }
  ...
  /**
   * Select/switch to a specific session.
   */
  ...
  /**
   * Send a message (called by the orchestrator for agent responses).
   */
  async sendMessage(_jid: string, text: string): Promise<void> {
    ...
    // Use the most recently added entry (last message sent triggers response)
  ...
  /**
   * Write response for a specific request (called after agent completes).
   */
  writeResponseForRequest(requestId: string, text: string, sdkSessionId?: string): void {
    // Check if we need to create/update CLI session
    if (sdkSessionId) {
      const existing = listCLISessions().find((s) => s.sdkSessionId === sdkSessionId);
      if (!existing) {
  ...
  /**
   * Sync session after container returns newSessionId.
   * Called by orchestrator when newSessionId is received.
   */
  ...
  /**
   * Write a response to the output directory.
   */
